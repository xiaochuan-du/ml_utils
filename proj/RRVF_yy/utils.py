...
def data2fea(src_df, data_dir, run_para={}, is_test=False, drop_vars=None):
    ...
    if not use_cacheing:
        ...
        data = {
            ...
            'prop': pd.read_csv('{}/air_visit_data_with_prop.csv'.format(data_dir))
        }
#         if is_test:
            # fill visits
...
#         tidy_df = add_rolling_stat(src_df)
        ...
        tidy_df = add_area_loc_stat(src_df, data)
...
#         tidy_df = tidy_df.assign(visit_date_ts=tidy_df.visit_date.astype('int') ) 
        prop = data['prop']
        prop['visit_date'] = pd.to_datetime(prop.visit_date)
        mat = pd.merge(tidy_df, data['prop'].drop(
            columns=['Unnamed: 0']),
            how='left',
            left_on=['air_store_id', 'visit_date', 'visitors'],
            right_on=['air_store_id', 'visit_date', 'visitors'])
        import time
        mat['visit_date'] = mat.visit_date.astype('str')
#         mat['visit_date_ts'] = mat.visit_date.apply(lambda x: int(time.mktime(time.strptime(x, "%Y-%m-%d"))))
        mat = mat.drop(['visit_date'], axis=1)
#         display(mat)
    else:
...
def test2fea(src_df, data_dir, run_para={}, is_test=False, drop_vars=None):
    " data cleansing and enrichment"
    af_etl = run_para.get("af_etl", None)
    use_cacheing = run_para.get("use_cacheing", None)
    if not use_cacheing:
        ...
        data = {
            ...
            'id': pd.read_csv('{}/store_id_relation.csv'.format(data_dir)),
            'hol': pd.read_csv('{}/date_info.csv'.format(data_dir)),
            'prop': pd.read_csv('{}/submission_data_with_prop.csv'.format(data_dir))
        }
#         if is_test:
            # fill visits
#             src_df = add_default_2_tst(src_df, data['tra'])
